...
%macro ALLOC 2			;reg, num/reg (can be the same)
	...
	sub qword [rel gc_stack], 0
	mov rax, [rel gc_stack]
	call print_rax
	sub qword [rel gc_stack], 0 ;0 vs rax; should be identical, but not
	;; jz %%do_work
	pushfq
	pop rax
	call print_rax
...
start:
	...
	mov rax, 0
	mov qword [rel tospace_bottom], 0	;there's some bizarre bug where "cmp qword [rel gc_stack], 0"
	;; or "sub qword [rel gc_stack], 0" doesn't set
...
lookup:
	...
	cmp rax, -1
	je lookup_right
	mov rax, [rdx+32]	;luckily, at this point CF will be set for 1, i.e. not set.
	ret
lookup_left:
	MOVE_AVL_NODE [rdx+8]
	mov rdx, rax
...
avl_add_dca:	
	...
	mov rdi, r8		  ;str
	add rax, 8
	add rdi, 8		;now things are tagged
...
make_avl_rebalance_sl_b:
	...
	mov rdx, [rcx+32]
	xor r15, [rel rcx_r10_avl] ;we stopped caring about rcx and r10 simul'ly.
	jmp make_avl
