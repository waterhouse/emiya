...
reverse_ret:
	...
	;; we will use gettimeofday.
	;; (facepalm)
	;; it produces two ints.
	;; so that'll be 8 bytes.
	;; --actually, bigger.
bad_get_nanoseconds:
	sub rsp, 16
	lea rdi, [rsp]
	mov rsi, 0		;hope this is NULL
	mov rax, 0		;in case
	call [gettimeofday]
	;; at this point, rsp=[sec|usec].
	mov rax, [rsp]
	mov rdi, 1000000
	mul rdi			;result nicely in rax
	mov rdi, [rsp + 8]
	add rax, rdi
	mov rdi, 1000
	mul rdi			;sigh
	;; result in rax
	;; need to readjust rsp
	add rsp, 16
	ret
