        ...
        ;; Next step: Basically I should do a mostly-comprehensive
        ;; text replacement of r15 and r14 with r11 and r10.
        ;; (As registers that allocation and barrier/moving operations may destroy.)
        ;; This will make it easier for the programmer to use C programs: there will
        ;; be now three registers that don't need to be saved/restored around C calls.
        ;; (Unrelatedly, rdtsc appears to be a count of CPU cycles.  On a laptop at
        ;;  2.2 GHz, it increments by roughly 2.2 billion every second.)
        ;; It also sort of makes it use the C calling convention: r10 and r11 are scratch.

        ;; Now we shall have a generic "get_nanoseconds" thing.
        ;; It'll become "mach_absolute_time" under OS X, something else under Linux,
        ;; and possibly something else under generic POSIX.
        ;; ... Turns out Racket doesn't distinguish between "unix" and any other Unix than OS X.
        ;; So...
        ;; Yeah, OS X (at least 10.6.8) doesn't have clock_gettime.
        ;; But it seems Linux and OpenBSD both have it...
        ;; That would be a good choice, but I'd need to find the CLOCK_REALTIME constant.
        ;; As I thought, it's 0 on Linux... and on OpenBSD.
        ;; I would expect it to be that in general...
        ;; I don't think I can conveniently have nasm read C header files for the constants.
        ;; A solution would be, as I did another time, to generate a C program that prints
        ;; out the value of CLOCK_REALTIME, compile it, run it, take the output, and use it.
        ;; But I don't wanna go to all that trouble yet.
        ;; For now...
        ;; I'll assume CLOCK_REALTIME is 0.  Beheheh.
        ;; Also, this should go in a separate file.

        ;; ...
        ;; Some of this requires shit to be done at startup.
        ;; (In particular, grabbing sysfuncs.)
        ;; In that case, we should maybe have a macro for adding things to the startup_sequence.
        ;; (A couple of macros: the user would define a macro that expanded to shit that
        ;;  needed to be run at startup, then would define startup_step_N to be its name and
        ;;  increment a startup_step_count number or something.  Some of *that* could be
        ;;  expressed using macros...)
        ;; ... Actually, the sysfuncs need to happen in a "c_save/c_restore" enclosure.
        ;; So, for now, neh.
        ;; --Well.
...
;;         etc.
        ...
        ;; Time to use specially sized registers in some places.
        ;; Similar to something done in x264.
        ...
        %define GC_SCRATCH r11
        %define GC_SCRATCH_2 r10
        ...
        ;; data_q %1, 0
        ;; they should all be consecutive
...
;;         ;; we can use r10 and r11 as we wish.
...
;;         ;;  return addresses and shit.]
        ...
        ;; better not be either rax or r11
...
%%again:
        ...
        ret
%%flip:
        ;; gc_flip still wants dick in r11
        mov r11, [rsp]
        call gc_flip
...
;;         add r11, ALLOCPTR
;;         mov PAGELIMIT, r11
;;         cmp r11, [tospace_top]  ;would be a GC flip, or would die if both need gc flip and gc work
;;         jng alloc_overflow_%1_noflip
;;         mov r11, [rsp]          ;used to compute el dicko
;;         call gc_flip
...
possibly_move_rdi:      
        ...
        ;; So.
        ;; We'll want to save r11.
        ;; --Nah.
        ;; push r11
        ;; call move_rdi
        ;; ret

        ;; In fact, we need to save rax, though.
        push rax
        call move_rdi
        pop rax
        ret
        ...
        ;; at least r11. good, we can use that.
move_rdi:
        ...
        mov eax, edi
        lea r11, [move_rdi_dispatch]
        and eax, 7
        
        jmp [r11 + 8*rax]
        ;; replacing that with this produces much smaller GC flip latency????
        ;; I guess the SIB byte circuits are stored several hours away by train?
        ;; shl rax, 3
        ;; jmp [r11 + rax]

        ;; ... So I realized there was one other effect: the effect on rax.
        ;; That shouldn't make a difference, but maybe I've fucked up.
        ...
        ;; --Ok, yeah, it's whether rax gets shifted that makes the difference.
        ;; So, then.
        ;; Why the fuck is that happening?  It shouldn't be.
        ;; Well, it must be something calling move_rdi...

        ;; I see.
        ;; Corruption from GC flip calling possibly_move_rdi.
        ;; It uses rax to store a mask thing.
        ;; So.
        ;; The move will, generally speaking, destroy rax, because it's convenient and
        ;; because
        ;; Therefore, possibly_move_rdi will have to save rax.
        ;; (GC flip could also be reworked to hold the mask somewhere other than rax.)
...
not_lucky:
        ...
        mov [r11 - CONS_TAG], r10
        mov r10, [rdi - CONS_TAG + 8]
        mov [r11 - CONS_TAG + 8], r10
        ;; make fwd ptr
        mov r10, r11
        bts r10, 63
        ;; install
        mov [rdi - CONS_TAG], r10
        ;; GC stack now.
        ;; Maybe bad register scheduling.
        mov r10, [gc_stack]
        mov [rdi - CONS_TAG + 8], r10
        ;; d'oh, forgot this
        mov [gc_stack], rdi
...
move_rdi_sym:
        mov r10, [rdi - SYM_TAG]
        fwd_ptr_test r10        ;less than 0?
        jl sym_maybe_lucky
sym_unlucky:
        add qword [moved_count], 1
        add qword [bytes_moved], 24

        lea r11, [ALLOCPTR + SYM_TAG]
        add ALLOCPTR, 24
        cmp ALLOCPTR, [tospace_top]
        ja out_of_memory
        ;; copy
        ...
        mov r10, [rdi - SYM_TAG + 16]
        mov [r11 - SYM_TAG + 8], rax
        mov [r11 - SYM_TAG + 16], r10
        ;; fwd ptr
        mov r10, r11
        bts r10, 63
        mov rax, [gc_stack]
...
work_and_grab_page:
        ;; So.
        ;; We can destroy r11 and probably rax.
...
trace_sym:      
        ...
        ;; move_rdi destroys: r10, r11, rax in the case of sym...
...
trace_words_detagged:   
        ...
        ;; r10 or r11 registers that we'll probably reserve for read barrier moving shit.
...
;;         %endmacro
        ...
        ;; %2 better not be rax or r11, %1 better not be r11
        %macro car 2
        mov al, %2b
        and al, 7
        cmp al, CONS_TAG
...
move_rdi_at_r11:
        push r11                ;lel, prob. fix later
        call move_rdi
        pop r11
        mov [r11], rdi
        ret

        define_move_at_r11 r8
        define_move_at_r11 rcx
        define_move_at_r11 rbp
        define_move_at_r11 rsi

        ;; get_nanoseconds macro.
        ;; Return value in rax.
        ;; Will destroy registers that C calls destroy.
        %ifdef DARWIN
        %macro get_nanoseconds 0
        call [mach_absolute_time]
        %endmacro
        %else                   ;use clock_gettime
        %macro get_nanoseconds 0
        call use_clock_gettime
        %endmacro
use_clock_gettime:
        ;; clock_gettime(clock_id, timespec_dest_ptr)
        sub rsp, 16
        mov rdi, 0              ;we're assuming CLOCK_REALTIME is 0
        mov rsi, rsp
        call [clock_gettime]    ;result = [seconds][nanoseconds]
        mov rax, [rsp]
        mov rdi, 1000000000
        mul rdi
        ;; that'll overflow a 64-bit register in probably year 2554
        add rax, [rsp+8]
        add rsp, 16
        ret
        %endif
...
plain_ret:      ret
        ...
        ;; r11 contains our "return address" thing.
...
gc_flip:
        ...
        ;; r11 contains our address thing.
        ;; find_saved_regs wants rdi as the argument
        ;; and destroys rsi and rcx.
        ;; Ok, make it take and return r11, instead.
        push rsi
        push rcx
        ...
        ;; It destroys, at the moment: r10 and r11.
...
trace_stack_next:       
        ...
        ;; ok, so... rdi and rax are being saved.
        ;; can already afford to hit anything move_rdi hits.
        ;; therefore:
        ;; must save rdx and rsi.
        ;; push rdx
        ;; push rsi
        ;; lea rdx, [gced_vars]
        ;; mov rsi, (gced_variable_count * 8)
        ;; call trace_words
        ;; pop rsi
        ;; pop rdx
        ;; geh, it doesn't work
...
dlsym:  dq 0
        ...
        %rep 8
        dq 0
        %endrep                 ;just in case isolating things works...
