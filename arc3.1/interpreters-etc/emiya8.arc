...
;EEEEEAAAAAGHHHHH
;back to Hikari to Yami

;Note this is currently not up to spec:
;If (meh) is a macro expanding to 'plus,
;and (plus . args) -> `(+ ,@args),
;then ((meh) 1 2) does become (+ 1 2) -> 3,
;which is good.
;But if (meh) -> 'fn or similar, then
;((meh) (x) (+ x 2)) doesn't work like (fn (x) (+ x 2)).
...
(= type-tag (obj int 0 cons 1 string 3 code 5 gc-work-cons 7))

;oh god symbols wtf do I do
;I need a symbol table
;suddenly symbol-value lookups will become faster
;I can fucking punt with symbol table as a completely flat list
;each symbol is basically [string-ptr] [symbol-value]
;reading, and gc finding roots, are the things the symbol-table is useful for.
;the latter does not require efficient random lookup.
;the former ... can be somewhat ignored for the moment.

;I'll tag system-functions (compiled-functions, or pointers thereto) as something special.
;Also, to handle return addresses: For the moment, in general, I'll just,
;when tracing the stack, not follow any pointers outside of both semispaces,
;and expect them to point to return addresses or summat. (Or C objects.)
;For the moment, I can just ignore strange pointers; that will suffice.
;Later, I could have it know where all return addresses are, in an eq-hash or
;summat, and maybe all other stationary objés. (As for return addresses of code
;that is GC'd, ... there is still opportunity to do the weird things like
;ensuring that all return addresses in GC'd code have the right low bits,
;... though... if you wanted to push a wrapper that wasn't a return address in
;itself, then returning from funcalls would have to be more complicated than
;just RET, in which case you'd probably have to wrap the native functions as well...
;Alternate strategy is to have a registry of all compiled code managed by GC,
;and maybe to push markers on the stack or summat. Then when the stack is traced...
;yeah.)
;When serializing dicks, you will have to map all native return addresses to
;their returning continuation-equivalents; can only print "<ptr to eval code
;right after this call", not "<memory address>".

;Ok, this stuff is more substantial and interesting than symbols.
;I'll use this.
;Internal calls are baked in.
;If I want to overwrite those, then I have a few options.
;- Do some wacko thing involving overwriting jump/call addresses.
;[This might not be possible given sizes.]
;- Create a new set of core code with baked-in internal calls, then
;  at some point replace it wholesale. This will require the return continuations
;  to be compatible.
;- ... Feh, whatever. (Could also replace it one return-address at a time.)

;Ok, so.
;Will have a couple of things mapping [instruction lists] <==> [fake memory addrs].
;Turns out that "far call" has basically the same interface
;(jump to destination, at destination the return address will be on the stack)
;as normal call.
;... WTF... why isn't there just a 64-bit RET?
;Looks like far calls to functions will have to work like this:
;<code segment 1>
;caller:
;CALL FAR dick
...
;giant dicks... and how about tail calls?
;seems those just work similarly.
;if func can jump short to code,
;then func can still jump short to code,
;and then the return address will be to dick,
;and the RET FAR will be appropriate.
;only thing is that there will be extra fuck on the stack.
;Fuck, maybe this is why people use RBP and crap.
;Well, neh, not too necessary.
;All pointers to objects on the stack will still be correct,
;and anything func reserves for itself on the stack will still be correct.
;All func can't do is assume that [RSP + n] is what it thinks it is,
;above a certain boundary.
;Only functions that would do this are those that assume information
;about their callers. I.e. functions that are compiled together, or at
;least compiled after previous ones and assume they won't change (and
;are prepared to have that assumption violated). This probably means
;it will have enough information to know whether the code chunks are
;within 2G of each other. ... But when serializing all compiled code
;with its assumptions... and then reloading it, who knows whether
;stuff will be loaded into the same stuff. I suppose it must be possible.
;Jesus christ. Maybe the loader would then be very complicated.
;Or I should serialize while removing all assumptions.
;Yeah, I should probably do the latter. (Goddammit.)

;Incidentally, a quick peek reveals that SBCL uses an "explicit PUSH and JMP"
;form of calling... the object PUSHed is "#<FDEFINITION object for B>",
;and it seems to be stored in the compiled code of A (which calls B),
;and [obj + 9] is apparently the address in the fdefinition object.
;Perhaps I should figure out whether PUSH;JMP is actually slower than CALL.
;Very well, done. (See call-chaining, and to a lesser extent {call,jmp}-testing.)


;On an unrelated note, it seems to me it might be a good idea to make
;"if" and "fn" and so forth be bound to "special form" objects, which
;are then handled specially by the interpreter; rather than having the
;symbols "if" and "fn" and so on being handled specially by the interpreter.
;... ... ... I think that may actually make things easier.
;I won't have to symbol-ify yet. Lolz so good/bad.
;And at any rate it will be in line with certain simplifications.
;Dick is incorporated into heh6.
;In our case, of course, the special objects can simply be pointers
;to, e.g., "eval-if" and "eval-fn" and so on. Or, perhaps, tagged indices
;into some sort of array...

;If I feel like customizing envs to be optimally fast, probably they should
;become vectors whose car is the parent env and length is known statically.
;... lolz... gc... how does gc figure out? by tracing the closure.

;... closures... even if crap gets redefined, closures will have the same
;env saved. only possible difficulty is if, due to re-macex, a value was
;not saved and is now used. (could an overeager idiot show that, due to
;current values of glob vars, a value should not be saved? and then that
;be changed? hmph.

;Some consideration yields problems with redefining functions, as well.
;(E.g. if you make-counters, then change how the counters are represented...
;and if the accesses or the updates are done with calls to globally defined
;functions, then you'll fuck your shit up.) So I'm emboldened to just have
;macros get applied as normal. The programmer can deal with it. (Compiler
;warnings could apply.)


;I think I will give "if", etc., the values of small negative integers
;with a "code" tag. Eventually this could be replaced with actual addresses
;of "if", etc.-handling code (which might become unnecessary when the damn
;thing is compiled), but for now, this. This has the advantage that the
;small integers can be represented directly in machine code (as bytes, in fact).
;... Well, it all worked. Next is symbols... I feel I might want to overwrite
;some things a few times. ...
;A global-value table will have to be initialized somehow.
;Also I'd probably want to have it start out being an alist and replace it
;later. Then ... ...
;I may have a register that serves as an array of addresses of subroutines
;that I might want to overwrite later.
;Because I can't trust relative offsets in nasm. Also I can't necessarily
;depend on loading pointer values into code; doing so at runtime requires
;write permissions to the code, and doing so at "load time" requires cooperation
;from the loader. Methinks I'll use "lea rax, [rel symbol]" to make a table
;of addresses.
;On a random note, it is not true that RAX contains the number of arguments
;passed to a varargs function, in C. RAX contains the number of "vector arguments".
;I.e. arguments put in strange registers no one ever uses. Ahem.
;Anyway, it should always be 0 when calling C crap. It appears C functions
;will find other means to know how many arguments are passed. (printf sees string)
;I see. Yes, there is no official functionality for varargs; user must explicitly
;pass n, and can easily pass an incorrect n.

;So... next difficulty will be symbols. And maybe it'll have to be done at
;the same time as reading... and therefore port management?...
;Bwahaha, I could write procedures to deal only with the direct
;stdin/out ports. I.e. would just use syscalls.
;Ports... interesting... because I want those to get closed upon GCment.
;Hmm. Based on easiness... It might be easier to make a symbol table,
;and only later make symbols. Yeah. Very well.
;emiya-symbols will probably be (arc-cons "symbol-name" <symbol-value>)
;just to absolutely baby my implementation abilities...

;Or, dunno, whatever.
;Eh, might as well start with something acceptable... the avl.
;Hence, user-defined types.
;Hence, the ... oh dear. Well, there we go...
;I should probably do as much with high-level language as possible, for
;as long as possible. Ehhhhnd there we shall go.
;--Hmm.
;So, eventually, the base system will have a base environment
;in which some symbols are bound.
;In other words, it will have a non-empty symbol table.
;Am I going to write a bunch of BS code to initialize the table?
;Or am I going to, say, load it from somewhere?
;I think the latter is better, although... sucks...
;Parse?
;I think ...
;I will certainly need parsing code at some point.
;... oh, and hey. t is a symbol.
;Ok, yes, things are more lively and good when stored formats
;are readable text. Also more fixable.
;Certainly the basic environment should be like that...

;For convenience in reading, I am introducing "shorten" ;and not really "lengthen"
;string functions. If threads eventually happen, then I suppose they
;will either a) be implemented with some kind of lock or b) not guarantee
;modification of the original string (like nreverse, use the result).
;Anyway, ideally, "shorten" would just overwrite the length field to
;something smaller, and "lengthen" would _ideally_ be done at a string
;allocated from the bottom of the heap (and which therefore ended at
;empty space). ... Fuck, that wouldn't really be doable; I currently
;move objects to the bottom of tospace, so ... this'd probably be
;quite special-purpose code or summat. Neh. But "shorten" is most certainly
;doable. Ideally the space after it would be reused. Could do that immediately:
;cut it and turn the rest into a string and return that ptr (could be empty string).

;

(= global-value (table))
(= special-forms '(if quote = fn mc quasiquote unquote))
(on s special-forms
  (let obj-name (symb s '-object)
    (= symbol-value.obj-name (+ type-tag!code (* -8 (+ 1 index)))
       global-value.s symbol-value.obj-name)))
;yes, a type tag for code even though it won't be used yet...
...
(def in-main-memory (x)
  (and (<= 0 x) (< x memory-top)))
...
;ok, so, fake memory addresses for code.
;I'll put them in the negative range. whatever.
;(This makes it easy for them to not be overlapped with main memory.
;The OS would automatically allocate BS elsewhere... I suppose I could
;do that. Neh, later.)
(def has-pointer-type-tag (x)
  (and (isa x 'int)
       (in (mod x 8)
           type-tag!cons
           type-tag!string
           type-tag!code
           type-tag!gc-work-cons)))
;"labels" will now map labels to fake instruction addresses,
;which will be mapped to cons cells, which can be mapped back
;to fake instruction addresses if desired.
;...
;instruction lists' cdrs will still be cons cells.
;... Never mind! cdrs are next instr address (or nil).

(= fake-memory (table)
   fake-memory-bottom -1000
   fake-alloc-pointer fake-memory-bottom)

(def in-fake-memory (x)
  (<= fake-alloc-pointer x fake-memory-bottom))

(defmemo fake-address (x) ;so ridiculous
  (let u (++ fake-alloc-pointer (-:+ 8 (rand 10)))
    (= fake-memory.u x)
    u))
(def next-instr (addr)
  (cdr fake-memory.addr))
;many things point to nil... oh well
;in general, shared cdrs... happen but are not a problem.
;(amusing, could conceivably be used to save code space)


(= the-pc nil
   the-stack nil
   labels (table)
   meta ($:make-weak-hash)) ;addr -> (name ... source)

(def install (body) ;such horrible metadata at the moment ;WELL IT'S GETTING WORSE ;or not ;or yes
        ...
        (do (= (labels car.xs) ys)
          (xloop (ys ys)
            (when ys
              ...
              (next next-instr.ys)))
          (next cdr.xs ys))
        (withs (u (cons (eval `(fn () ,car.xs)) ys)
                a fake-address.u)
          (push car.xs meta.a)
          (next cdr.xs a)))))
...
(def step ()
  (++ step-counter)
  (let u fake-memory.the-pc
    (zap next-instr the-pc)
    (car.u)))
(= ustep-table (table))
(= ustep-list nil)
(def ustep ()
  ...
  (w/uniq gargs
    `(do (install '(,name ,@body))
       (= ,name (fn ,gargs
          ...
          (fn ,gargs
            (make-assignments ',args ,gargs)
            (push fake-address.arc-return the-stack)
            (jump ',name))))))

(def thunkify (expr)
  (eval `(fn () ,expr)))
(= arc-return (map thunkify '((prn return))))
...
(ddef eval-if (xs env) ;xs, env are uconses
  ...
  (= x ucar.xs) ;sixth...
  (push next-instr.the-pc the-stack) ;not a ucons
...
#;(ddef uapply-loop (env exprs) ;DICKASS IDIOT DON'T ELIMINATE TAIL CALLS YOU IDIOT
  ...
  (save env ucdr.exprs)
  (= x ucar.exprs)
  (push next-instr.the-pc the-stack)
...
(def gc-flip ((o get-more-memory get-more-memory-next-time))
  ...
  ;Ok, so... we move things that are well-tagged ptrs in normal memory
  ;and ignore things that are tagged ptrs but are in fake memory
  ;(i.e. where immobile code is kept)
  ;and flip tables if we find things that are tagged ptrs but are
  ;in 
  (xloop (x the-stack)
    (when x
      (let p car.x
        (when has-pointer-type-tag.p
          (if in-fake-memory.p
              (do (when stack-loud (prsn "NOT MOVING APPARENT RET ADDR" p))
                (unless fake-memory.p (prsn "HEY WHAT IS THIS" p)))
              fromspace-pointer.p
              (do (when stack-loud (prsn "MOVING THING ON STACK" p))
                (zap move car.x))
              no:in-main-memory.p
              (prsn "WHERE IS THIS" p))))
      (next cdr.x)))
