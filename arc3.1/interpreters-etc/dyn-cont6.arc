...
(def ucall (f xs d e k)
  (vcase f
    ...
    ;qcompose (ueval-compose xs d e k)
...
(each x '(if quote quasiquote fn #;mc dlet assign #;compose)
...
(each x
  '(+ - * /
    ...
    ;this is just written this way for convenience
    (special-object? (auto-nerb
                      [and cons?_ (is car._ 'special-value)]))
    
    (zero? (auto-nerb ($:lambda (x) (if (zero? x) 't 'nil))))
    (even? auto-nerb.even)
    
    
    ;no quasiquote object, that's a macro
    (fn-object qfn)
    ;k, we're not doing compose-object atm
    ;(compose-object '
