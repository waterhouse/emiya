...
(= uinsult-toplevel (bclos (fn (self _) ;forgivable here because this is just one func.
...
(uinstall 'safe-call/kd (bclos (fn (self k d (k2 d2 thunk))
...
(def cont->closure-c1 (self ignk ignd (x))
  (bcall e.0 x))

(uinstall 'ccc (bclos (fn (self k d (f))
                        (let u cont->closure.k
                          (uapply d k f list.u)))))
(uinstall 'raw-ccc (bclos (fn (self k d (f))
                            (uapply d k f list.k))))

(uinstall 'eval (bclos (fn (self k d (x (o e nil)))
                         (ueval d e k x))))

(uinstall 'apply (bclos (fn (self k d (f . args))
...
(= default-uhandler (bclos (fn (self k d (obj))
...
(each x '(+ - * / < >
          div mod)
  (uinstall x (uarg-tck 'int proc->bclos:symbol-value.x)))

#;(each x '(car cdr cadr cddr)
  (uinstall x (uarg-nck 1 (uarg-tck 'cons proc->bclos:symbol-value.x))))
;that's actually not too great
;...
;lel, just drop type check
(each x '(car cdr cadr cddr)
  (uinstall x (uarg-nck 1 proc->bclos:symbol-value.x)))

(each x '(< >)
  (uinstall (symb x 2)
            (uarg-nck 2 (uarg-tck 'int proc->bclos:symbol-value.x))))

(uinstall 'string-length (uarg-tck 'string proc->bclos.len))

(each x '(is)
  (install-uassignment x proc->bclos:symbol-value.x))
...
(uinstall 'arc-eval (bclos (fn (self k d (x))
