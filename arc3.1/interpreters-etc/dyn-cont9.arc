...
(def uapply (d k f args)
  (let u base-tag.f
    (let addr (uapply-base-table u [uraise d k 
                                           (list 'apply-unknown-error __)])
      (addr d k f args))))
...
(def apply-user (d k f args)
  (let u user-obj-tag.f
    (let addr (apply-user-table u [uraise d k
                                          (list 'apply-unknown-user-error __)])
      (addr d k f args))))
...
(def umac? (x) (and user?x (is user-obj-tag.x 'umac)))
...
(def utype (x)
  (let u base-tag.x
    (case u
      user (err "Can't be bothered, but this should look up into a table" x)
      u)))
...
(def boot ()
  (let n 0
    (let fuck-me nil
      (fromfile (string src-directory "arc-boot.arc")
        (whilet u (read)
          (= last-u u)
;          prn.n
...
(def prettify (x)
  (deep-map [if usym?_ usym->sym._
                user?_ `(,upcase:user-obj-tag._ ,@(prettify user-obj-val._))
                _]
            x))
