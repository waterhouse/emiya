...
;^ Hah.  The idiot me forgot about dynamic variables in the above
;list, it would seem.  Also using vectors.  And, for full Arc
;compliance, complex function things--NO THOSE CAN BE MACROS.
;Ok, did that last one.
;Vectors would be used to represent closures... and envs would
;be sub-things.  Now...
;[Note a user can easily turn fn back into its old form, perhaps
; controlling this with a parameter... and I could do that sort
; of thing to reduce boot time.]

;Btw, "alist" should be renamed, perhaps to "a-list", and "alist"
;should construct an alist.

;^ OR WE SHOULD USE BETTER SSYNTAX.
...
(= obj-name ($.make-hasheq) ;was code-table, but need if-objs, etc.
   obj-name!HELLA-NIL nil)
...
;no worries about being able to read, for now
(def uwrite (x)
  (write-graph x))
...
(each (x y) (tuples 2 '(+ + - - * * / / cons cons car car cdr cdr is is < < > >
  ...
  (= global-value.x symbol-value.y
     (obj-name symbol-value.y) x))

(each s '(quote if = fn mc quasiquote arc unquote)
  (let name (symb s '-object)
    (= symbol-value.name (list 'special-form (string s))
       global-value.s symbol-value.name
       (obj-name symbol-value.name) s)))
...
(no:ueval '((fn ()
              ...
              (mac w/uniq (v . body) ;wootz we are defining this after
                (if (acons v)        ;its use in for
                    ...
                    `(let ,v (uniq) ,@body)))
              ;here... I can account for optionals, and can even
              ;account for destructuring.
              ;--as long as I don't introduce infinite loops...
              (= underlying-fn fn)
              ...
              (mac unless (test . body)
                `(if ,test nil (do ,@body)))
              (mac arc-with (varvals . body)
                `((fn ,(map1 car (tuples 2 varvals)) ,@body)
                  ,@(map1 cadr (tuples 2 varvals))))
              ;the below is amazingly slow, but works (a compiler would
                    ...
                    (w/uniq gargs
                      (xloop (args args plains nil)
                        (if (is-optional (car args))
                            `(underlying-fn ,(flip plains gargs)
                               ,(xloop (args args)
                                  (if (no args)
                                      `(do ,@body)
