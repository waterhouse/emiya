...
(def uapply (f xs k) ;really doesn't need k; remove later
  #;(prsn 'uapply f xs)
  (if (is-system-proc f)
      (call-closure k (apply f xs))
      (and acons.f (is car.f 'closure))
      (call-closure k (apply-closure f xs))
                                 ...
                                 (list 'closure
                                       (fn (ev b)
                                         (call-closure (alref ev 'k)
                                                       (cons (alref ev 'a)
                                                             b)))
                                       (make-alist 'k (alref ev 'k)
                                                   'a a))))
                     (make-alist 'rest rest 'env env 'k k))))))
;Eh, let's do it the other way, yeah.
#;(def map-eval (xs env k)
  (if no.xs
      (call-closure k nil)
      (let (x . rest) xs
        (let cdr-clos (list 'closure
                            (fn (ev b)
                              (call-closure (alref ev 'k)
                                            (cons (alref ev 'a)
                                                  b)))
...
(= global-value!apply (fn (f . xs)
                        (apply uapply f (join xs list.idfn-closure)))
