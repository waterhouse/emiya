...
(def dppr (x (o ind 0) (o nosp t))
  ...
  (aif atom.x
       ...
       (do (pr "(")
         (dppr car.x (+ ind 1) t)
         (sp)
         (dppr-list cdr.x (+ ind 1) nil) ;muahaha ;noob you forgot
         (pr ")")) ;derf
       (is car.x 'let)
       (do (pr "(")
         (dppr-list x ;(cons (+ ind 5) (+ ind 2))) ;aww yeah
                    (apply list* (map [+ _ ind] '(1 4 4 2)))) ;beheh
         (pr ")"))
       (do (pr "(")
         (dppr-list x (cons (+ ind 1) (+ ind 2)))
         (pr ")"))))
...
(def dppr-list (xs inds (o lastpr nil))
  (if no.xs
      ...
      (let ind (if acons.inds pop.inds inds)
        ...
        (dppr pop.xs ind t) ;dumbass forgot dick
      ...
      (let ind (if acons.inds pop.inds inds)
        ...
        (when atom.xs (pr ". "))
        (dppr (if acons.xs car.xs xs) ind t) ;no popping, must keep info
