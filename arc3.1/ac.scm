...
;the following two lines are used in my profiling[n].arc stuff
;(require "terrible-define2.rkt")
;(define setters terrible)
;gloriously terrible oh my god

(define a (compile-enforce-module-constants))

(define (tell-me-a)
  a)

#;(define (redefine-symbol x v)
  (namespace-set-variable-value! x v))

#;(define tns (current-namespace))

;(compile-enforce-module-constants #f)

;oh dear, the below several lines disagree with a new Racket
...
;(define b (compile-enforce-module-constants))

;(require racket)
;(compile-enforce-module-constants #f)
;Wootz!  Turns out the above does not seem to cost anything in runtime.
;... nor does it seem to work. jesus.
...
(define (expand-ssyntax sym)
  ((cond ((or (insym? #\. sym) (insym? #\! sym)) expand-sexpr) ;###HACK### switched the order
      ...
      (let ((b (ac b1 (ac-dbname! a env))))
        (list 'let `((zz ,b))
               (cond ((eqv? a 'nil) (err "Can't rebind nil"))
                     ;((eqv? a 't) (err "Can't rebind t")) ;###HACK###
                     ((lex? a env) `(set! ,a zz))
...
(define (ac-call fn args env)
  (let ((macfn (ac-macro? fn)))
    (cond ((eqv? fn 'funcall) ;###FOR PURPOSES OF AWESOME###
           (map (lambda (x) (ac x env)) args))
          ((and macfn (not (lex? fn env))) ;###HACK###
...
(define (ar-funcall1 fn arg1) ;moar speed hack: data structure lookups
                              ;the generational GC actually works here, though,
                              ;so it doesn't make all that big a difference.
                              ;still, useful to minimize malloc noise to see
                              ;just how much anything else mallocs. also, might
                              ;become important if I can tighten loops.
  ;arc> (time:$:let loop ((x 0)) (if (> x 100000000) 'nil (begin (ar-funcall1 _alphabet 10) (loop (+ x 1)))))
  ;before: time: 3783 real: 3769 gc: 732 mem: -14939456
  ;after:  time: 2407 real: 2404 gc: 0 mem: 936
  (cond ((procedure? fn)
         (fn arg1))
        ((pair? fn) 
         (list-ref fn arg1))
...
; for fast binary calls;
; like lambda except the additional first argument is used for
; the binary case
(define-syntax binary-lambda
  (syntax-rules ()
    ((_ binary-f args . body)
     (case-lambda
       ((x y) (binary-f x y))
       (args . body)))))
#;(define-syntax binary-lambda
  (syntax-rules ()
    ((_ ignored . args) (lambda . args))))
...
(xdef is (binary-lambda ar-is2 args (pairwise ar-is2 args)))
...
(xdef + ;(or +  ;bwahahahahaha
  (binary-lambda ar-+2 args
           (cond ((null? args) 0)
...
(xdef > (binary-lambda ar->2 args (pairwise ar->2 args)))
...
(define (stupid-value x) ;is this needed? just imitating "namespace-set-variable-value"
  (namespace-variable-value x #t (lambda () 'nil)))
  ...
  (on-err (lambda (c) 
    ...
    (lambda ()
      (let ((expr (read)))
        (if (eqv? expr ':a)
            'done
            (let ((tm (current-milliseconds))
                  (ps (current-process-milliseconds))
                  (gc (current-gc-milliseconds))
                  (mm (current-memory-use)))
              (let (;(n (next-char-place))
                    (val (arc-eval expr)))
                ;the timing data also counts compilation time; little I'd like
                ;to do about that
                (let ((dmm (- (current-memory-use) mm))
                      (dgc (- (current-gc-milliseconds) gc))
                      (dps (- (current-process-milliseconds) ps))
                      (dtm (- (current-milliseconds) tm)))
                  ;(if (< n (next-char-place))
                  ;    (newline))
                  (write (ac-denil val))
                  (namespace-set-variable-value! '_thattime (list dtm dps dgc dmm))
                  (namespace-set-variable-value! '_thattt (stupid-value '_thatt))
                  (namespace-set-variable-value! '_thatt (stupid-value '_that))
                  (namespace-set-variable-value! '_that val)
                  (namespace-set-variable-value! '_thatttexpr (stupid-value '_thattexpr))
                  (namespace-set-variable-value! '_thatttexpr (stupid-value '_thatexpr))
...
; we are committed to mzscheme v4+ and will use its unsafe
; mutable-pair operations to force the mutation of a pair.
