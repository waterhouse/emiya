...
;  ###NO! That would only work by modifying the cons cell that (car x) is,
;  which is not what push normally does.
...
;NOTE: NESTING THESE THINGS IS O(3^N) IN GENERATED CODE SIZE.
...
(def union (f xs ys)
  (join xs (rem (fn (y) (some [f _ y] xs))
             ys)))
